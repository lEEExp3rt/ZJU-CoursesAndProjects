...
uint64_t do_mmap(struct mm_struct *mm, uint64_t addr, uint64_t len, uint64_t vm_pgoff, uint64_t vm_filesz, uint64_t flags) {
    uint64_t _sva = PGROUNDDOWN(addr); // Start of the VMA after rounding down.
    uint64_t _eva = PGROUNDUP(addr + len); // End of the VMA after rounding up.
    uint64_t _npg = (_eva - _sva) / PGSIZE; // Number of pages in the VMA.

    // Insert the new VMA into the VMA list.
    struct vm_area_struct *vma = (struct vm_area_struct *)alloc_page(); // Allocate a new VMA.
    vma->vm_start = addr;
    vma->vm_end = addr + len;
    ...
    vma->vm_flags = flags;
    vma->vm_next = NULL;
    struct vm_area_struct *head = mm->mmap; // Header of the VMA list.
    if (!head) {
        mm->mmap = vma;
...
uint64_t nr_tasks = 2;              // Currently running thread count
...
void load_program(struct task_struct *task) {
    ...
    for (int i = 0; i < ehdr->e_phnum; ++i) {
        ...
        if (phdr->p_type == PT_LOAD) {
            uint64_t perm = (
                phdr->p_flags & PF_R ? VM_READ  : 0 |
                phdr->p_flags & PF_W ? VM_WRITE : 0 |
                phdr->p_flags & PF_X ? VM_EXEC  : 0
            );
