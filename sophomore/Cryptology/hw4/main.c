...
//#2_begin------------
    // Expand 32-bit input to 48-bit
    byte tmp1[8][6], tmp2[8];
    // Initialize
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 6; j++)
            tmp1[i][j] = 0;
    ...
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 6; j++) {
            ...
            tmp1[i][j] |= (bufin[index / 8] & bytebit[index % 8]) ? 1 : 0;
        }
    }
    // Xor
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 6; j++) {
            tmp2[i] |= (tmp1[i][j] << (5 - j)) ^ (subkey[i] & bytebit[j + 2]);
        }
    }
    // Sbox & Pbox
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 4; j++)
            bufout[j] |= sbox_output_perm_table[i][tmp2[i]][j];
    }
//#2_end==============
}
